...
// ===== Index page popup (no redirect, just closes) =====
(function () {
  const path = window.location.pathname;
  const isHome = /(^\/$|index\.html$)/.test(path); // ONLY / or index.html
...
// ===== Lander page popup (redirect on Yes/No) =====
(function () {
  const path = window.location.pathname;
  const isLander = /lander\.html$/.test(path); // ONLY lander.html
  if (!isLander) return;
